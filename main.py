...
def extract_domain_from_rule(rule):
    ...
    - @@domain.com (例外规则)
    - @@||domain.com (例外规则)
    - 0.0.0.0 domain.com (空IP映射)
    - domain1.com,domain2.com,domain3.com##.element (通用隐藏规则)
    """
    # 移除规则选项部分 (以$开头的部分)
    base_rule = rule.split('$')[0]

    # 处理 0.0.0.0 格式 (通常形式为 "0.0.0.0 domain.com")
    if base_rule.startswith('0.0.0.0 '):
        base_rule = base_rule[8:]  # 移除 "0.0.0.0 "
    
    # 处理 @@ 格式的例外规则
    if base_rule.startswith('@@'):
        base_rule = base_rule[2:]  # 移除 @@
        
    # 处理 @@|| 格式
    if base_rule.startswith('||'):
        base_rule = base_rule[2:]  # 移除 ||

    # 处理通用隐藏规则格式 (domain1.com,domain2.com,domain3.com##.element)
    if '##' in base_rule:
        # 分离域名部分和选择器部分
        domains_part = base_rule.split('##')[0]
        # 按逗号分割多个域名
        domain_parts = domains_part.split(',')
...
def get_ip_location(ip_str):
    ...
    with IP_LOCATION_CACHE_LOCK:
        ...
        # 检查缓存大小，如果超过限制则清理
        if len(IP_LOCATION_CACHE) >= MAX_CACHE_SIZE:
            # 清理一半的缓存项
            keys_to_remove = list(IP_LOCATION_CACHE.keys())[:MAX_CACHE_SIZE // 2]
...
def get_domain_ips(domain):
    ...
    with DNS_CACHE_LOCK:
        ...
        if len(DNS_CACHE) >= MAX_CACHE_SIZE:
            # 清理一半的缓存项
            keys_to_remove = list(DNS_CACHE.keys())[:MAX_CACHE_SIZE // 2]
            for key in keys_to_remove:
                del DNS_CACHE[key]
